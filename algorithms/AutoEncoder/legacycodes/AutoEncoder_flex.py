...
if len(layer) == 3:
    ...
    print('Training AutoEncoder of layer {}-{}-{}-{}, max epoch {}'.format(layer[0], layer[1], layer[2], layer[3], num_epochs))
    ...
    if epoch % n_print == 0:
        print(
            'Finished epoch [{}/{}], training time {:.2f}s. Training loss:{:.4f}, Validation loss:{:.4f}'.format(
                epoch + 1, num_epochs, end - start, loss_train[epoch, 0], loss_valid[epoch, 0]))
