    ...
    def __init__(self, nodenum):
        ...
        if len(nodenum) == 2:
            self.encoder = nn.Sequential(
                nn.Linear(226, nodenum[0]), nn.ReLU(True),
                nn.Linear(nodenum[0], nodenum[1])
            )
            self.decoder = nn.Sequential(
                nn.Linear(nodenum[1], nodenum[0]), nn.ReLU(True),
                nn.Linear(nodenum[0], 227), nn.Sigmoid()
            )
        elif len(nodenum) == 3:
            self.encoder = nn.Sequential(
                nn.Linear(226, nodenum[0]), nn.ReLU(True),
            ...
            print('Currently not supporting encoding layer other than 2/3/4/6/10')
