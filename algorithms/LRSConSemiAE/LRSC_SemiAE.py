...
def encodingX(X, modelpath):
    ...
    Xae = torch.from_numpy(X).to(device)
...
filetitle = 'SemiAE_LRSC_Model_VH_raw.p'
...
print('SemiAE raw LRSC Model accuracy: {:.2f}/{:.2f}/{:.2f}'.format(score_train_lrsc_raw*100,
                                                             score_valid_lrsc_raw*100,
                                                             score_test_lrsc_raw*100))

datapath = '../../data/all/all_IO_noleave_VH.npz'
dataset = np.load(datapath)
files_VH = [f for f in os.listdir('../SemiAutoEncoder/') if f.endswith('VH.pt')]
for i in range(len(files_VH)):
    modelpath = '../SemiAutoEncoder/'+files_VH[i]
    model_param = torch.load(modelpath)
    ...
    filetitle = 'SemiAE_LRSC_Model_VH_' + nodenum + '.p'
...
filetitle = 'SemiAE_LRSC_VH_Performance.p'
...
filetitle = 'SemiAE_LRSC_Model_N_raw.p'
pickle.dump(lrsc_raw, open(filetitle, 'wb'))
...
datapath = '../../data/all/all_IO_noleave_N.npz'
dataset = np.load(datapath)
files_N = [f for f in os.listdir('../SemiAutoEncoder/') if f.endswith('N.pt')]
for i in range(len(files_N)):
    modelpath = '../SemiAutoEncoder/'+files_N[i]
    model_param = torch.load(modelpath)
    layer = model_param['layer']
    if len(layer) == 2:
        nodenum = '{}-{}'.format(layer[0], layer[1])
    ...
    filetitle = 'SemiAE_LRSC_Model_N_' + nodenum + '.p'
    pickle.dump(lrsc_encode, open(filetitle, 'wb'))
...
filetitle = 'SemiAE_LRSC_N_Performance.p'
