...
def load_data(name, n=None):
    ...
    :return: np array
    """
    all_data = np.load('../../data/all/all_IO_noleave_VH.npz')
...
class Config(object):
    ...
    def __str__(cls):
        return "{}-{}-{}-{}-{}-{}-{}-{}".format(
            cls.S1*2, cls.S2*2, cls.S3*2, cls.C1, cls.C2, cls.C3, cls.C4, cls.D_out
        )
...
class DotaNet(torch.nn.Module):
    def __init__(self, config):
        ...
        self.synergy_net = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Linear(config.S2, config.S3),
            torch.nn.ReLU()
        )

        # layers of counter network
        self.counter_net = torch.nn.Sequential(
            torch.nn.Linear(config.S3*2, config.C1),
            torch.nn.ReLU(),
            ...
            torch.nn.ReLU(),
            torch.nn.Dropout(0.5),
            torch.nn.Linear(config.C3, config.C4),
            torch.nn.ReLU(),
            torch.nn.Dropout(0.5),
            torch.nn.Linear(config.C4, config.D_out),
            torch.nn.Sigmoid()
        )
...
def startNN():
    ...
    torch.save(performances, '{}_VH.pt'.format(config))
...
def draw_chart(performances, config):
    ...
    plt.title('Neural Network with Shared Weights \n layer {} on VH Data, test accuracy: {:.4f}'.format(
        config, performances['accuracy_test']
    ))
    plt.legend(loc='upper left')
    plt.plot()

    plt.savefig('accuracy_{}_VH.png'.format(config))
